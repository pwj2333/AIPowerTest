...
export default function ResultsPage() {
  ...
  return <div className="admin-page">
    ...
    <div className="filter-bar"><label className="filter-select" htmlFor="results-campaign-select">测评批次<select id="results-campaign-select" aria-label="结果测评批次" value={campaignId} onChange={(event) => setCampaignId(event.target.value)}><option value="">请选择批次</option>{campaigns.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label className="search-field"><Search size={16} /><input aria-label="搜索员工" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索姓名或岗位" /></label><label className="filter-select"><Filter size={15} /><select aria-label="结果部门" value={department} onChange={(event) => setDepartment(event.target.value)}><option>全部部门</option>{departments.map((item) => <option key={item}>{item}</option>)}</select></label><label className="filter-select"><select aria-label="结果等级" value={level} onChange={(event) => setLevel(event.target.value)}><option>全部等级</option>{Array.from({ length: 8 }, (_, index) => <option key={index + 1} value={index + 1}>L{index + 1} {getGrade(index + 1).name.replace("等级", "")}</option>)}</select></label>{hasFilters && <button className="clear-filter" type="button" onClick={() => { setDepartment("全部部门"); setLevel("全部等级"); setQuery(""); }}><X size={14} /> 清除筛选</button>}</div>
